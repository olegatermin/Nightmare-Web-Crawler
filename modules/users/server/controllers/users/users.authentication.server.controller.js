 ...
 * API signin
 */
exports.signinApi = function (req, res, next) {
  // `local` strategy requires usernameOrEmail field to be provided
  req.body.usernameOrEmail = req.body.username;
  passport.authenticate('local', { session: false }, function (err, user, info) {
    if (err || !user) {
      ...
      res.json({
        status: 'success',
        auth_token: jwt.sign(_.pick(user, ['_id', 'password', 'provider', 'roles']), config.jwt.secret)
      });
    }
  })(req, res, next);
};
